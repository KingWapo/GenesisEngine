...
class Process;
typedef std::tr1::shared_ptr<Process> StrongProcessPtr;
typedef std::tr1::weak_ptr<Process> WeakProcessPtr;
...
public:
	enum State
	{
		...
		REMOVED, // removed from the process list but not destroyed; this can happen
		// when a process that is already running is parented to another process
...
protected:
	...
	virtual void VOnFail(void) { } // called if the process failes (see below)
