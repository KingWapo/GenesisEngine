	...
	Point &result)
{
	...
	/*
	* Computer a1, b1, c1, where line joining points 1 and 2
	* is "a1 x + b1 y + c1 = 0".
	*/
	...
	// Check signs of r3 and r4. If both point 3 and point 4 lie on the
...
//
// bool Intersect - Returns true if the supplied rect is inside a circular radius.
//
bool Intersect(const Rect &rect, const Point &center, double const radius)
{
	...
	if (r.right < 0)		/* R to left of circle center */
	{
		if (r.bottom < 0)	/* R in lower left corner */
			return ((r.right * r.right + r.bottom * r.bottom) < radiusSquared);
		else if (r.top > 0)	/* R in upper left corner */
			return ((r.right * r.right + r.top * r.top) < radiusSquared);
	...
	else if (r.left > 0)	/* R to right of circle center */
	{
		if (r.bottom < 0)	/* R in lower right corner */
			return ((r.left * r.left) < radiusSquared);
		else if (r.top > 0)	/* R in upper right corner */
			return ((r.left * r.left + r.top * r.top) < radiusSquared);
...
//
// void Interpolate -- Interpolates a point
//
